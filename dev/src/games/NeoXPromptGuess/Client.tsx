...
import { PlayerHandle, CaptionData, TextCompletion } from './GameTypes'
	...
	return (
      ...
      <Switch fallback={<p>Invalid game state.</p>}>
        ...
        <Match when={gameState() === GameState.CreatingImages}>
          ...
          <input id="diffusionPrompt" placeholder="Once upon a time"></input>
          <button onclick={() => diffuse()}>Generate!</button>
        </Match>
        
        <Match when={gameState() === GameState.CaptioningImages}>
          <Show when={captionTexts()[0].player.uuid !== playerHandle()?.uuid}
